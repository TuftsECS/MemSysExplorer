#!/bin/bash
# Compiler-flag sweep for CPU2017 fprate, parallelized with GNU parallel.
...
#   MAAP_OUT=$HOME/maap_out_cpu2017 ./run_sweep.sh
#
...
# is copied to.
CFG="$(dirname "$(readlink -f "$0")")/cpu2017.cfg"

MAAP_OUT="${MAAP_OUT:-$HOME/maap_out_cpu2017}"
...
run_one_tag() {
  ...
  MONITOR_LOG_ROOT="$tag_root/monitor_logs" \
    runcpu --config="$CFG" \
           --define maap_out_cpu2017="$tag_root" \
           --define label="$tag" \
           --define optflag="$flag" \
           --action=run \
           --noreportable \
